...
img = io.imread('results/borderMasks/FT_500X_A_3_border.jpg')

#Look at the dimensions
print(img.shape)  #3 dimensions (height, width, channels)

# Convert to grayscale only if the image is RGB (has 3 channels)
if len(img.shape) == 3 and img.shape[2] == 3:
    DAPI = rgb2gray(img)
else:
    DAPI = img
print(DAPI.dtype) #float64. Need to convert to 8 bit
